...
import { ref } from "vue";
import { ElButton, ElCheckbox, ElInput, ElIcon, ElMessage } from "element-plus";
...
const handleLogin = async () => {
    ...
    if (res.data.code == 200) {
        ...
        localStorage.setItem('username', loginForm.value.username); // 存储用户名[10](@ref)
        ElMessage.success('登录成功！');
        ElMessage.success(`欢迎 ${loginForm.value.username} 使用 RBAC 权限管理系统！`); // 直接使用模板字符串[7](@ref)
        router.push('/Home'); // 跳转主页
        loginLoading.value = false; // 启用加载动画
