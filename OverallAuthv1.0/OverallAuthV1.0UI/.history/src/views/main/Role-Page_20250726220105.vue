<template>
    ...
    <div class="role-content">
        ...
        <div class="content" v-loading="loading">
            <el-scrollbar max-height="550px">
                <el-table :data=newdata border style="width: auto;" stripe ref="multipleTableRef">
    ...
    <!-- 编辑角色对话框 -->
    <el-dialog v-model="dialogEditVisible" title="编辑角色" width="30%" draggable center>
...
const fetchRoleData = async () => {
    try {
        ...
        if (res.data.code === 200) {
            ...
            total.value = data.value.length; // 更新总条数
        } else {
            ElMessage.error('获取角色数据失败:', res.data.message);
        }
    } catch (error) {
...
const Editdialog = async (row: Role) => {
    ...
    // 1. 清理无效菜单名（空字符串、null、undefined）
    const validMenuNames = row.menusName.filter(name => Boolean(name));

    // 2. 若全无效则直接返回空数组
    if (validMenuNames.length === 0) {
        DialogEditform.value.checkedKeys = [];
        treeRef.value?.setCheckedKeys([]); // 强制清空组件内部状态
        treeRef.value?.store?.setCheckedKeys([]); // 操作内部Store
        await nextTick(() => {
            treeRef.value?.setCheckedKeys([]); // 强制清空组件内部状态
...
const EditRole = async () => {
    try {
        if (DialogEditform.value.name === '') {
            ElMessage.warning('角色名是必填项!!');
        } else {
            const res = await axios.post("http://127.0.0.1:5141/api/Role/EditRole?id=" + DialogEditform.value.id, {
                name: DialogEditform.value.name,
                describe: DialogEditform.value.describe,
                menuids: DialogEditform.value.checkedKeys,
                isEnable: DialogEditform.value.isEnable
