...
const Editdialog = async (row: Role) => {
    ...
    // 1. 定义动态key
    const treeKey = ref(0); // 初始值
    // 2. 刷新组件的函数
    const refreshTree = () => {
        treeKey.value += 1; // 修改key值（或使用 Math.random()）
    };
    // 2. 若全无效则直接返回空数组
    if (validMenuNames.length === 0) {
        ...
        await nextTick(async () => {
            ...
            if (store) {
                ...
                // 递归清除所有节点（包括未渲染的懒加载节点）
                const clearNodeState = (node) => {
                    node.isChecked = false;
                    if (node.childNodes) node.childNodes.forEach(clearNodeState);
                    if (node.hasOwnProperty("lazy")) {
                        node.loaded = false; // 关键：标记为未加载[4](@ref)
                    }
                };
                clearNodeState(store.root);
            }
